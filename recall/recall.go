...
func init() {
	...
	engine.OnMessage(zero.NewPattern().Reply().Text("^(recall|撤回)$").AsRule(), zero.OnlyGroup, zero.AdminPermission).SetBlock(true).Handle(func(ctx *zero.Ctx) { //这个正则还是有缺陷的, 比如这样的消息: [CQ:reply,id=123]]]]]] 112233 依然会匹配. 脑细胞不太够用,想不出什么合适的解决方法
