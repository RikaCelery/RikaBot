...
func init() {
	...
	*/
	engine.OnMessage(zero.NewPattern(nil).Text(`^/(\S+)$`).At().AsRule()).SetBlock(true).Handle(func(ctx *zero.Ctx) {
	...
	engine.OnMessage(zero.NewPattern(nil).Text(`^/(\S.+)$`).AsRule(), func(ctx *zero.Ctx) bool {
		model := extension.PatternModel{}
		_ = ctx.Parse(&model)
	...
	engine.OnMessage(zero.NewPattern(nil).Reply().Text(`^/(\S+)$`).AsRule()).SetBlock(true).Handle(func(ctx *zero.Ctx) {
		model := extension.PatternModel{}
		_ = ctx.Parse(&model)
