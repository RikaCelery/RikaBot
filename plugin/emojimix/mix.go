...
func init() {
	mixLimiter := rate.NewManager[string](time.Minute*1, 2)
	mixCommandLimiter := rate.NewManager[string](time.Second*10, 2)
...
{prefix}可选
    ...
    加 每10秒 每个人 最多触发 2 次
    管理员使用{prefix}无视限制`,
	}).OnMessage(match).SetBlock(true).Limit(func(ctx *zero.Ctx) *rate.Limiter {
...
const webp = `https://fonts.gstatic.com/s/e/notoemoji/latest/%s/512.webp`
...
func emojiHashToWebpUrl(hashSlug string) string {
	return fmt.Sprintf(webp, strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(hashSlug, "-ufe0f", ""), "u", ""), "-", "_"))
}
