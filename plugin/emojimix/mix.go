	...
	e.OnMessage(match, func(ctx *zero.Ctx) bool {
		...
		Handle(func(ctx *zero.Ctx) {
			...
			if local := path.Join(e.DataFolder(), slug+".gif"); utils.Exists(local) {
				...
				return
			} else if local = path.Join(e.DataFolder(), strings.ReplaceAll(slug, "-ufe0f", "")+".gif"); utils.Exists(local) {
				bytes, err := os.ReadFile(local)
				if err != nil {
					ctx.Send(fmt.Sprintf("ERROR: %v", err))
					return
				...
				ctx.Send(message.ImageBytes(bytes))
				return
			} else if local = path.Join(e.DataFolder(), slug+"-ufe0f.gif"); utils.Exists(local) {
				bytes, err := os.ReadFile(local)
				if err != nil {
					ctx.Send(fmt.Sprintf("ERROR: %v", err))
					return
				...
				log.Infof("local animated sticker not found (%s)", slug)
			}
